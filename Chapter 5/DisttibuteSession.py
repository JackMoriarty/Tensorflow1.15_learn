...
flags.DEFINE_string("train_dir", "/tmp/mnist-log",
                    "Directory for storing checkpoint and summary file")
flags.DEFINE_integer("num_gpus", 0, "gpu number")
...
def main(unused_argv):
    ...
    with tf.device(
        ...
        # 使用relu作为激活函数，hid作为隐层输出
        hid = tf.nn.relu(hid_lin)
        # 定义softmax的输出y， 即为推理计算的标签值
    ...
    if FLAGS.sync_replicas:
        ...
        sess_config = tf.ConfigProto(
            ...
            log_device_place_placement=False,
            device_filters=["/job:PS", "/job:worker/task:%d" % FLAGS.task_index])
        
        # 如果是chief worker， 则初始化所有worker的分布式会话
        if is_chief:
            print("Worker %d: Initializing session..." % FLAGS.task_index)
        # 如果是其他worker，则等待chief worker返回的会话
        else:
        ...
        # 如果是同步更新模式，并且当前进程为chief work
        if FLAGS.sync_replicas and is_chief:
            # 初始化同步标记队列
            sess.run(sync_init_op)
            # 通过QueueRunner启动3个线程，并运行各自的标准的服务
            sv.start_queue.runners(sess, [chief_queue_runner])

        # 记录并打印训练开始前的时间
        time_begin = time.time()
        print("Training begins @ %lf" % time_begin)
        # 将local_step 赋值为0
        local_step = 0
        while True:
            # 填充训练数据
        ...
        # 填充验证数据
        val_feed = {x: mnist.validation.images, y_: mnist.validation.lables}
        # 在验证数据集上计算模型的交叉熵
